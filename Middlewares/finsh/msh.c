...
int msh_help(int argc, char **argv)
{
    // rt_kprintf("RT-Thread shell commands:\n");
    {
        ...
        for (index = shell->syscall_table_begin;
                index < shell->syscall_table_end;
...
void msh_auto_complete_path(char *path)
{
    ...
    if (*path != '/')
    {
        ...
        if (full_path[rt_rt_strlen(full_path) - 1]  != '/')
            strcat(full_path, "/");
    }
    else *full_path = '\0';
    ...
    else
    {
        ...
        for (;;)
        {
            ...
            if (rt_strncmp(index, dirent->d_name, rt_rt_strlen(index)) == 0)
            {
                if (min_length == 0)
                {
                    min_length = rt_rt_strlen(dirent->d_name);
                    /* save dirent name */
        ...
        if (min_length)
        {
            if (min_length < rt_rt_strlen(full_path))
            {
                /* list the candidate */
                rewinddir(dir);
...
void msh_auto_complete(char *prefix)
{
    ...
    {
        for (index = shell->syscall_table_begin; index < shell->syscall_table_end; FINSH_NEXT_SYSCALL(index))
        {
