...
// RT_SECTION(".shared_ram") struct finsh_shell _shell;
...
int finsh_getchar(void)
{
    ...
    // extern int8_t rt_hw_console_getchar(void);
    // return rt_hw_console_getchar();
    int8_t ch = shell->getchar();
    return ch;
...
{
    rt_base_t level;
    rt_size_t pw_len = rt_rt_strlen(password);

    if (pw_len < FINSH_PASSWORD_MIN || pw_len > FINSH_PASSWORD_MAX)
        return -RT_ERROR;

    level = rt_hw_interrupt_disable();
    rt_rt_strncpy(shell->password, password, FINSH_PASSWORD_MAX);
...
static void finsh_wait_auth(void)
{
    ...
    rt_size_t cur_pos = 0;
    /* password not set */
    if (rt_rt_strlen(finsh_get_password()) == 0)
        return;
...
RTM_EXPORT(finsh_thread_entry);
void finsh_system_function_init(const void *begin, const void *end)
{
    shell->syscall_table_begin = (struct finsh_syscall *)begin;
    shell->syscall_table_end = (struct finsh_syscall *)end;
}
...
#endif
#include "timer.h"
struct rt_timer timer_shell;
struct finsh_shell def_shell;
/*
 * @ingroup finsh
 *
...
void finsh_system_init(void)
{
    ...
    shell->echo_mode = 1;
    extern int8_t rt_hw_console_getchar(void);
    shell->getchar = rt_hw_console_getchar;

#endif /* RT_USING_HEAP */

    finsh_set_prompt_mode(1);
    rt_kprintf(FINSH_PROMPT);
    rt_timer_init(&timer_shell, finsh_thread_entry, RT_NULL, 10, RT_TIMER_FLAG_PERIODIC);
    rt_timer_start(&timer_shell);
}
...
// {
//     usart_write((rt_uint8_t *)str, len);
// }
