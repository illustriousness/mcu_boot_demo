...
#define FLASH_OFFSET 24 * 1024 //30KB boot (240-30)=210kb app 240-256 16KB arg. app bl 各一半

extern int onchip_flash_read(uint32_t offset, uint8_t *buf, uint32_t size);
extern int onchip_flash_write(uint32_t offset, const uint8_t *buf, uint32_t size);
extern int onchip_flash_erase(uint32_t offset, uint32_t size);
...
uint32_t crc32_check(uint32_t crc, uint8_t *pdata, uint32_t len)
{
    ...
    for (i = 0; i < len; i++)
    {
        crc = (crc >> 8) ^ crc32_table[(crc & 0xff) ^ pdata[i]];
    }
...
size_t _fdb_set_status(uint8_t status_table[], size_t status_num, size_t status_index)
{
     ...
     * ------------------------------------------------------------------------------------------------------
     * |   16bit    | 0xFFFF 0xFFFF 0XFFFF| 0x0000 0xFFFF 0XFFFF| 0x0000 0x0000 0XFFFF | 0x0000 0x0000 0X0000
     * ------------------------------------------------------------------------------------------------------
     ...
     */
    rt_memset(status_table, FDB_BYTE_ERASED, FDB_STATUS_TABLE_SIZE(status_num));
    if (status_index > 0)
    {
#if (FDB_WRITE_GRAN == 1)
...
fdb_err_t
_fdb_write_status(fdb_db_t db, uint32_t addr, uint8_t status_table[], size_t status_num, size_t status_index, int sync)
{
...
#elif defined(FDB_USING_OTHER_MODE)
        if (onchip_flash_read(FLASH_OFFSET + addr, buf, size) < 0)
        {
            result = FDB_READ_ERR;
        }
#else
...
#elif defined(FDB_USING_OTHER_MODE)
        if (onchip_flash_erase(FLASH_OFFSET + addr, size) < 0)
        {
            result = FDB_ERASE_ERR;
        }
#else
...
fdb_err_t _fdb_flash_write(fdb_db_t db, uint32_t addr, const void *buf, size_t size, int sync)
{
    fdb_err_t result = FDB_NO_ERR;
...
#elif defined(FDB_USING_OTHER_MODE)
        if (onchip_flash_write(FLASH_OFFSET + addr, buf, size) < 0)
        {
            result = FDB_WRITE_ERR;
        }
#else
