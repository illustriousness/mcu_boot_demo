...
/* the file cache table size, it will improve GC speed in file mode when using
 * cache */
...
#define FDB_ASSERT(EXPR)                                                   \
    if (!(EXPR))                                                           \
    {                                                                      \
        FDB_LOG_E(db, "(%s) has assert failed at %s.\n", #EXPR, __func__); \
        while (1)                                                          \
            ;                                                              \
    }
*/
#define FDB_ASSERT(EXPR)
#endif /* FDB_ASSERT */
// #define FDB_INFO(...) rt_kprintf(__VA_ARGS__)
#define FDB_INFO(...)
...
{
    enum fdb_kv_status status; /**< node status, @see fdb_kv_status_t */
    int crc_is_ok;             /**< node CRC32 check is OK */
    uint8_t name_len;          /**< name length */
    uint32_t magic;            /**< magic word(`K`, `V`, `4`, `0`) */
    uint32_t len; /**< node total length (header + name + value), must align by
                   FDB_WRITE_GRAN */
...
typedef int (*fdb_tsl_cb)(fdb_tsl_t tsl, void *arg);
...
{
    FDB_SECTOR_DIRTY_UNUSED, // trigger chk gc flag
    FDB_SECTOR_DIRTY_FALSE,  // gc flag done
    FDB_SECTOR_DIRTY_TRUE,   // chk ok,
    FDB_SECTOR_DIRTY_GC,     // in gc
...
{
    int check_ok; /**< sector header check is OK */
    struct
    {
...
typedef struct fdb_db
{
    ...
    uint32_t empty_kv; /**< the next empty KV node start address */
    int8_t init_ok;       /**< initialized successfully */
    int8_t file_mode;     /**< is file mode, default is false */
  // int not_formatable;   /**< is can NOT be formated mode, default is false */
#ifdef FDB_USING_FILE_MODE
...
struct fdb_kvdb
{
    ...
    int gc_request;                    /**< request a GC check */
    int in_recovery_check; /**< is in recovery check status when first reboot */
    struct fdb_kv cur_kv;
    struct kvdb_sec_info cur_sector;
    int last_is_complete_del;
...
struct fdb_tsdb
{
    ...
    int rollover; /**< the oldest data will rollover by newest data, default is
                   true */
